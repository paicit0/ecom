...
const verifyBearerAndIdtoken = async function (
  ...
  console.log("verifyBearerAndIdtoken: req.header: ", req.headers.authorization);
  ...
  if (!authHeader || !authHeader.toLowerCase().startsWith("bearer")) {
    ...
    res.status(401).json({ error: "verifyBearerAndIdtoken: no auth in headers!" });
    return;
  }
  ...
  try {
    ...
    console.log(error);
    res.status(401).json({ error: `verifyBearerAndIdtoken: Unauthorized! ${error}` });
    return;
  }
  next();
};
